...
interface Papa2SceneProps {
  onComplete: () => void
  isActive: boolean
}

export function Papa2Scene({ onComplete, isActive }: Papa2SceneProps) {
  ...
  const firstMessage = `
  Quizás las cosas no fueron como queríamos, pero sucedieron de la mejor manera, 
  pues aunque hoy ante las personas seas mi "padrastro", 
  quiero decirte que realmente para mí eres MI PADRE.
  Porque tienen razón, no es el que engendra, es el que cría.
  Y tú has hecho una labor impagable.
  Te quiero!
  `
  ...
  }, [firstTextComplete, isActive, onComplete]);
