...
const message2Video = videoFondos.parents;
const padres = event.parents;

interface PadresSceneProps {
  onComplete: () => void;
  isActive: boolean;
}

export function PadresScene({ onComplete, isActive }: PadresSceneProps) {
  ...
  const secondMessage = padres.mother;
  const thirdMessage = padres.father;
  const fourthMessage = `
  Mis amores más grandes son ustedes, mis padres. 
  Son mi guía, mi apoyo y mi hogar. ¡Los amo infinitamente!
  `;
